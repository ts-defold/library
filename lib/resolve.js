...
async function resolve(projectDir, types) {
    ...
    for (const dep of deps) {
        const index = await (await (0, node_fetch_1.default)(`${TYPES_REGISTRY}/${dep.name}/`)).json();
        ...
        if (!version)
            throw new Error(`No suitable version found for ${dep.name}`);
    ...
    for (const dep of hasTypes) {
        ...
        for (const v of prune) {
            ...
            if (!err)
                console.log(`Pruned types ${dep.name}@${v.version}`);
        }
    }
}
